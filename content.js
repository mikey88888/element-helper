...
class MainApp{
    ...
    async init(){     
     ...
     // v10.1 - 移除原有悬浮窗，改用操作面板
     // this.createNavbar();//调用函数创建导航栏  默认隐藏
     // this.toggleDiv();

     // v10.1 - 移除对 daohanglan 的引用（已删除）
     // $('#daohanglan').on('click', function() {
     //     togglePosition();
     // });

     // v10.1 - 移除鼠标移动监听（改用操作面板的选择模式）
     // this.listen_mousemove_to_update_div();
...
// 万能侧边栏圆形按钮
// v10.1.2 - 修复 iframe 多悬浮球问题：只在主页面创建侧边栏按钮
if (window === window.top) {
var side_button_code = `

    <div id='yuananniu' class="yuananniu" title="菜单">

        <ul id="lyrics-list">
            <li>元</li>
            <li>素</li>
        </ul>
        <div class="xiong-dropdown-menu">
            <div id="xiong_toggle_plugin" class="xiong-dropdown-item">禁用插件</div>
            <div id="xiong_open_panel" class="xiong-dropdown-item">打开操作面板</div>
        </div>
    </div>
    ...
    // 新的菜单项事件绑定
    $('#xiong_toggle_plugin').click(function() {
        // 调用 v10_1_enhancements.js 中的 togglePlugin 函数
        if (typeof togglePlugin === 'function') {
            togglePlugin();
        } else {
            console.error('togglePlugin 函数未找到');
        }
    });

    $('#xiong_open_panel').click(function() {
        // 调用 operation_panel 的 toggle 方法
        if (typeof operation_panel !== 'undefined' && operation_panel.toggle) {
            operation_panel.toggle();
        } else {
            console.error('operation_panel 未找到');
        }
    });
...
// 调用函数设置悬浮窗
// v10.1.2 - 修复 iframe 多悬浮球问题：只在主页面创建悬浮窗
if (window === window.top) {
    setupFloatingWindow();
}      
// 创建和配置悬浮窗
...
// 调用函数来启动歌词滚动效果
// v10.1.2 - 修复 iframe 多悬浮球问题：只在主页面启动滚动效果
